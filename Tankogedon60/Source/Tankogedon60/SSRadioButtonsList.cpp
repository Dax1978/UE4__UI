...
void SSRadioButtonsList::Construct(const FArguments& InArgs)
{
	...
	ChildSlot
	[
		SAssignNew(Holder, SVerticalBox)	// Работает практически как SNew, но не просто создает виджет, но и сохраняет его в переменную
											// Теперь в переменной Holder лежит наш созданный SVerticalBox
	];
...
//{
//	// Что бы слоты не добавлялись бесконечно, надо добавить проверки
...
TSharedRef<SWidget> SSRadioButtonsList::CreateCheckbox(int32 InIndex, FString InText)
{
	return SNew(SCheckBox)
		...
		// Здесь добавляем наш стиль для чекбокса
		.Style(CheckBoxStyle)
		[
			SNew(STextBlock)
			.Text(FText::FromString(InText))
			// Здесь добпавляем стиль для текстблока
